...
# This script uses transparent, standard Python routines for estimation,
# inference, diagnostics, sensitivity analysis, and output generation. The
# research design and all analysis settings are documented in README.md.
#
# Security design:
...
# Hourly time-zone alignment and the CAISO/EIA source merge are performed by
# prepare_analysis_data.py. This analysis script starts from one daily,
# analysis-ready CSV whose exact columns are listed in
# ANALYSIS_INPUT_DICTIONARY.csv.
########################################################################

# This block imports only the modules and aliases used by the analysis.
import sys
...
# using the seven daily lags fixed in the research design.
...
# This function calculates a correlation matrix with np.corrcoef. No variables
# are deleted because of a high correlation; the matrix is diagnostic evidence
# only.
...
# 30 days explicitly with shifts, slices, np.corrcoef, and a loop. This makes
# every lag calculation directly auditable.
...
def run_analysis(source_data, analysis):
    ...
    # This block calculates VIFs for the full regressor matrix using the
    # statsmodels variance_inflation_factor routine.
    ...
    # deterministic trend, and seven fixed lagged differences. These checks do
    # not convert the association design into causal inference.
    ...
    # studentized residuals. It does not classify a flag as an error and does
    # not delete or winsorise the primary data.
    ...
    # Monthly averaging uses groupby/mean and differencing uses diff. It is a
    # sensitivity, not the primary daily estimate.
...
# A clear error identifies a genuinely missing merged input rather than failing
# later inside pandas with an ambiguous path.
...
print(f'Input file: {INPUT_FILE.relative_to(PROJECT_FOLDER)}\n')
